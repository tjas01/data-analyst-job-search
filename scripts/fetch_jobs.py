...
Outputs:
  ...
  - data/jobs_digest.txt  (sorted most recent, plain text)
"""
...
def clean_html(raw: str) -> str:
    return re.sub(r(Â¼[^>]+>", "", raw or "").strip()[:300]
...
def fetch_jobbank() -> list[dict]:
    ...
    for keyword in JOBBANK_SEARCHES:
        q = keyword.replace(" ", "+")
        # Job Bank exposes an RSS feed via their search URL with &sort=M (most recent)
...
â¬Ã¢ââ¬Ã¢ââ¬Ã¢ââ¬Ã¢ââ¬Ã¢ââ¬Ã¢ââ¬Ã¢ââ¬Ã¢ââ¬Ã¢ââ¬Ã¢ââ¬Ã¢ââ¬Ã¢ââ¬Ã¢ââ¬Ã¢ââ¬Ã¢ââ¬Ã¢ââ¬Ã¢ââ¬Ã¢ââ¬Ã¢ââ¬Ã¢ââ¬Ã¢ââ¬Ã¢ââ¬Ã¢ââ¬Ã¢ââ¬Ã¢ââ¬Ã¢ââ¬
...
# BC and remote-friendly companies known to use Greenhouse
    ...
    for company in GREENHOUSE_COMPANIES:
        ...
        try:
            ...
            for job in data.get("jobs", []):
                ...
                if not normalize_title(title):
                    continue
                # Filter: Canada/BC or Remote location
...
# BC companies known to use Lever
LEVER_COMPANIES = [
    ...
    for company in LEVER_COMPANIES:
        ...
        try:
            ...
            for job in postings:
                ...
                loc_lower = (location + " " + commitment).lower()
                if not any(kw in loc_lower for kw in ["british columbia", "bc", "vancouver", "canada", "remote", "anywhere", ""]):
                    pass  # Lever often has empty location for remote, so include all
                seen_urls.add(link)
                jobs.append({
                    "id": make_id(link),
                    "title": title,
...
# BC and Canadian tech companies known to use Ashby
ASHBY_COMPANIES = [
    ...
    for company in ASHBY_COMPANIES:
        ...
        try:
            ...
            for job in data.get("jobs", []):
                ...
                loc_lower = location.lower()
                # Include remote, Canada, or BC roles
